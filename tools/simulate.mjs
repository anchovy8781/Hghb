...
for (let i = 0; i < runs; i++) {
  ...
  const ORI = [null, 'after18', 'coma', 'wed'];
  const opts = { origin: ORI[i % 4], longs: i % 2 === 1 };
  ...
  if (!ok) failed++;
  console.log(
    `시드 ${seed}${opts.origin ? (opts.origin === 'after18' ? '' : '(' + opts.origin + ')') : '(사연 고름)'}${opts.longs ? '(장편)' : ''} · ${r.page}p · ${r.chapter}장까지 · 인카운터 ${r.encounters} · ` +
