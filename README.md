...
1. **템플릿 덱** — 사건 템플릿을 비복원으로 뽑는다. 한 바퀴가 끝나기 전에는 같은 템플릿이 다시 나오지 않고, 최근 10개는 후보에서 제외된다.
2. **슬롯 조합** — 구역 20 · 장소 52 · 인물 이름 1,200 · 유형 16 · 특징 24 · 위협 16 · 아이템 63 을 각각 별도의 덱에서 뽑아 끼운다.
...
만들어질 수 있는 이야기 조합은 **약 1억 4천만 가지**(게임 내 설정 화면에서 확인 가능).

검증은 자동 플레이로 한다. `tools/simulate.mjs` 는 5000페이지를 끝까지 플레이하면서
**모든 페이지의 본문**을 해시로 비교한다. 한 번이라도 겹치면 실패로 끝난다.

```
$ node tools/simulate.mjs 6 500
시드 500 · 5015p · 10장까지 · 인카운터 1880 · 고유 본문 4940 · 중복 0 · 엔딩 복수
...
통과 6/6 — 중복 본문 없음
```
